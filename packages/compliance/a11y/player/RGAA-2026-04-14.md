...
| #    | Criterion     | Status | Observation                                                                                                                                                                                                                                                                                                                                                    |
| ---- | ------------- | ------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 10.7 | Focus visible | NC     | Computed focus style on all three control buttons is `outline-style: none` (the `outline-width: 3px` / `outline-color: #fff` declarations are therefore inert) with no `box-shadow` fallback. Keyboard users have **no visible focus indicator** on Play/Pause, Mute, or Fullscreen. Fix: remove `outline-style: none` on `:focus-visible`, or add `box-shadow`. |
...
| Critical | Restore a visible focus indicator on Play/Pause, Mute, Fullscreen (remove `outline-style: none`, or add a visible `box-shadow` / `outline` on `:focus-visible`).                                                                                              |
