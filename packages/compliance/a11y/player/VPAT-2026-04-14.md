...
- All three control buttons compute to `outline-style: none` on `:focus` with no `box-shadow` fallback. Keyboard users receive no visible focus indicator.
...
| 2.4.7         | Focus indicator suppressed (`outline-style: none`) on all control buttons                  |
