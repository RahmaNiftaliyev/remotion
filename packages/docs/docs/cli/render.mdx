...
[Render a subset of a video](/docs/config#setframerange). Example: `--frames=0-9` to select the first 10 frames. You can omit the start or end: `--frames=100-` renders from frame 100 to the end<AvailableFrom v="4.0.421" inline />, `--frames=-100` renders from the start to frame 100<AvailableFrom v="4.0.421" inline />. To render a still, use the `still` command.
...
### `--experimental-rspack`<AvailableFrom v="4.0.TBD" />
