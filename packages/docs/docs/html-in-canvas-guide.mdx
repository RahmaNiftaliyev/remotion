...
[HTML-in-canvas](https://github.com/WICG/html-in-canvas) is an experimental browser API currently only available in Chrome Canary behind a flag.
...
[`<HtmlInCanvas>`](/docs/remotion/html-in-canvas) is a convenient component which wraps it's children in a canvas and allows you to manipulate the content when the canvas is painted.
...
If you do so, you are responsible yourself for calling [`delayRender()`](/docs/delay-render) and [`clearRender()`](/docs/clear-render) to synchronize paint operations.
