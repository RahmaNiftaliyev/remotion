...
- License key
- Host domain (`window.location.origin`)
...
This data is **not shared with any third parties** — it is only used internally by Remotion for license compliance purposes.
...
From Remotion 5.0, opt-out is possible under an [Enterprise License](https://remotion.pro/license) agreement if monthly usage reports are provided instead. The Enterprise License has the same per-render pricing, but requires a minimum spend of $500 per month.
