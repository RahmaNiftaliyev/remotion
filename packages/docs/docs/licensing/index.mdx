...
**For [`@remotion/vercel`](/docs/vercel/api)**: Pass the `licenseKey` option to [`renderMediaOnVercel()`](/docs/vercel/render-media-on-vercel#licensekey) and [`renderStillOnVercel()`](/docs/vercel/render-still-on-vercel#licensekey) to trigger an event.
