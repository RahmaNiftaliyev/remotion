...
If the asset is returning a 404 status code, the file might not be served correctly.  
If the file is in your `public/` folder, you must use [`staticFile()`](/docs/staticfile) to reference it. A plain path like `"/my-video.mp4"` will not work because Remotion serves static files under a special prefix.

```tsx twoslash title="❌ Wrong - plain path"
import {Html5Video} from 'remotion';

<Html5Video src="/my-video.mp4" />;
```

```tsx twoslash title="✅ Correct - using staticFile()"
import {Html5Video} from 'remotion';
...
Also make sure the file actually exists in your `public/` folder and is spelled correctly.
