...
```ts twoslash title="Minimal crossfade example"
// @noErrors
import 'remotion';
...
```ts twoslash
import type {HtmlInCanvasShaderDraw} from '@remotion/transitions';
//                ^?
```

The argument has the type `HtmlInCanvasShaderDrawParams<Props>`:

```ts twoslash
import type {HtmlInCanvasShaderDrawParams} from '@remotion/transitions';
//                ^?
```
