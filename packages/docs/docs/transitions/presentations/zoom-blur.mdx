...
Because this presentation relies on [HTML-in-canvas](/docs/client-side-rendering/html-in-canvas), it only runs in Chromium-based browsers that ship the experimental `drawElementImage` and `canvas.requestPaint()` APIs. As of writing, this means [Chrome Canary](https://www.google.com/chrome/canary/) with the `chrome://flags/#canvas-draw-element` flag enabled. Server-side renders work the same way — pass `--allow-html-in-canvas` (or [`Config.setAllowHtmlInCanvasEnabled(true)`](/docs/config#setallowhtmlincanvasenabled)) and a Chrome Canary executable.
