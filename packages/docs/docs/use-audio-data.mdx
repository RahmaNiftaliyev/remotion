...
:::note
The optional `options` argument below requires **Remotion v4.0.456** or newer. That release is not out on npm yet — watch the [changelog](https://www.remotion.dev/changelog).
:::
...
### `options?`<AvailableFrom v="4.0.456"/>

#### `sampleRate?`<AvailableFrom v="4.0.456"/>
...
#### `requestInit?`<AvailableFrom v="4.0.456"/>
