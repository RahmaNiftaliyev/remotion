...
Re-exported from [`@remotion/renderer`](/docs/renderer/render-media). Contains the fields `renderedFrames`, `encodedFrames`, `encodedDoneIn`, `renderedDoneIn`, `renderEstimatedTime`, `progress`, and `stitchStage`.
