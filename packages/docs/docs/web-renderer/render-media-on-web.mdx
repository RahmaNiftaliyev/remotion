...
A **warning** is logged once per `renderMediaOnWeb()` run: either that the native path is in use for frames, or **`Not using html-in-canvas: …`** with a concrete reason. Use `logLevel: 'warn'` or lower if you do not see these messages.
