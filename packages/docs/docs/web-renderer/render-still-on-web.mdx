...
Default `false`. In the Remotion Studio, the checkbox under **Other** is initialized from [`Config.setAllowHtmlInCanvasEnabled()`](/docs/config#setallowhtmlincanvasenabled) and the [`--allow-html-in-canvas`](/docs/cli/studio#--allow-html-in-canvas) CLI flag:

<Option id="allow-html-in-canvas" />

When `true`, Chromium-based browsers that expose the experimental [HTML-in-Canvas](https://github.com/WICG/html-in-canvas) API (`drawElementImage`, enabled e.g. via `chrome://flags/#canvas-draw-element`) may rasterize the full composition frame with the browser engine instead of Remotion’s built-in DOM composer. This is usually faster but **pixels may not match** the software path.
...
<Option id="media-cache-size-in-bytes" />
