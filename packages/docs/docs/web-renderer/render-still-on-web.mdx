...
Renders a single frame in the browser. When the promise resolves, the frame is captured to an [`OffscreenCanvas`](https://developer.mozilla.org/en-US/docs/Web/API/OffscreenCanvas). You then choose how to read or encode it: as a canvas, [`Blob`](https://developer.mozilla.org/en-US/docs/Web/API/Blob), or object URL (similar to [Renoun’s `screenshot()` API](https://github.com/souporserious/renoun/tree/main/packages/screenshot)).
